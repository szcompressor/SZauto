...
sz_block_error_estimation_3d(const T * data_pos, const float * reg_params_pos, const float * reg_poly_params_pos,
        ...
        float noise, double& err_reg, double& err_lorenzo, double& err_reg_poly, const int pred_dim,
    ...
    if (pred_dim == 3) {
        if (use_lorenzo_2layer) {
            lorenzo_predict = lorenzo_predict_3d_2layer(cur_data_pos, dim0_offset, dim1_offset);
        }
        if (use_lorenzo){
        ...
        if (use_lorenzo_2layer) {
            lorenzo_predict = lorenzo_predict_2d_2layer(cur_data_pos, dim0_offset, dim1_offset);
        }
        if (use_lorenzo){
        ...
        if (use_lorenzo_2layer) {
            lorenzo_predict = lorenzo_predict_1d_2layer(cur_data_pos, dim0_offset);
        }
        if (use_lorenzo){
            lorenzo_predict = lorenzo_predict_1d(cur_data_pos, dim0_offset);
        }
    }
	err_lorenzo += mean_info.use_mean ? MIN(fabs(cur_data - mean_info.mean), fabs(cur_data - lorenzo_predict) + noise) : fabs(cur_data - lorenzo_predict) + noise;
}
...
sz_blockwise_selection_3d(const T *data_pos, const meanInfo<T> &mean_info, size_t dim0_offset, size_t dim1_offset, int min_size,
                          float noise, const float *reg_params_pos, const float *reg_poly_params_pos, const int pred_dim,
                          const bool use_lorenzo, const bool use_lorenzo_2layer, const bool use_regression,
                          const bool use_regression_poly) {
	double err_reg = 0;
	double err_lorenzo = 0;
    double err_reg_poly = 0;
	for(int i=2; i<min_size; i++){
		int bmi = min_size - i;
		sz_block_error_estimation_3d(data_pos, reg_params_pos, reg_poly_params_pos, mean_info, i, i, i, dim0_offset, dim1_offset, noise, err_reg, err_lorenzo, err_reg_poly, pred_dim, use_lorenzo, use_lorenzo_2layer, use_regression, use_regression_poly);
		sz_block_error_estimation_3d(data_pos, reg_params_pos, reg_poly_params_pos, mean_info, i, i, bmi, dim0_offset, dim1_offset, noise, err_reg, err_lorenzo, err_reg_poly, pred_dim, use_lorenzo, use_lorenzo_2layer, use_regression, use_regression_poly);
		sz_block_error_estimation_3d(data_pos, reg_params_pos, reg_poly_params_pos, mean_info, i, bmi, i, dim0_offset, dim1_offset, noise, err_reg, err_lorenzo, err_reg_poly, pred_dim, use_lorenzo, use_lorenzo_2layer, use_regression, use_regression_poly);
		sz_block_error_estimation_3d(data_pos, reg_params_pos, reg_poly_params_pos, mean_info, i, bmi, bmi, dim0_offset, dim1_offset, noise, err_reg, err_lorenzo, err_reg_poly, pred_dim, use_lorenzo, use_lorenzo_2layer, use_regression, use_regression_poly);
	}
    //TODO kai add weights for reg_poly
    bool use_lorenzo_all = use_lorenzo || use_lorenzo_2layer;
    if (use_regression && (err_reg <= err_lorenzo || !use_lorenzo_all) && (err_reg <= err_reg_poly || !use_regression_poly)) {
        return SELECTOR_REGRESSION;
    } else if (use_regression_poly && (err_reg_poly <= err_lorenzo || !use_lorenzo_all) &&
               (err_reg_poly <= err_reg || !use_regression)) {
