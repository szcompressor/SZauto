...
template<typename T>
unsigned char *
sz_compress_3d_knl(const T * data, size_t r1, size_t r2, size_t r3, double precision, size_t& compressed_size, int BSIZE3d){
	DSize_3d size(r1, r2, r3, BSIZE3d);
	int capacity = 0; // num of quant intervals
	meanInfo<T> mean_info = optimize_quant_invl_3d(data, r1, r2, r3, precision, capacity);
	int intv_radius = (capacity >> 1);
	...
	size_t reg_count = prediction_and_quantization_3d_with_knl_optimization(data, size, mean_info, precision, capacity, intv_radius, indicator, type, reg_params_type, reg_unpredictable_data_pos, unpred_count_buffer, unpred_data_buffer, est_unpred_count_per_index);
	unsigned char * compressed = NULL;
	// TODO: change to a better estimated size
	size_t est_size = size.num_elements*sizeof(T)*1.2;
	compressed = (unsigned char *) malloc(est_size);
	...
	for(int i=0; i<size.block_size; i++){
		for(int j=0; j<size.block_size; j++){
			cout << unpred_count_buffer[i*size.block_size + j] << " ";
			write_array_to_dst(compressed_pos, unpred_data_buffer_pos, unpred_count_buffer[i*size.block_size + j]);
			if(i==7 && j==7) cout << unpred_count_buffer[i*size.block_size + j] << " " << unpred_data_buffer_pos[758] << endl;
...
template 
unsigned char * 
sz_compress_3d_knl<float>(const float * data, size_t r1, size_t r2, size_t r3, double precision, size_t& compressed_size, int BSIZE3d);
