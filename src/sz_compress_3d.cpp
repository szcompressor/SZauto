    ...
    for (size_t i = 0; i < size.num_x; i++) {
        ...
        for (size_t j = 0; j < size.num_y; j++) {
            ...
            for (size_t k = 0; k < size.num_z; k++) {
                ...
                if (*indicator_pos) {
                    // regression
                    compress_regression_coefficient_3d(reg_precisions, reg_recip_precisions, reg_params_pos, reg_params_type_pos,
                                                       reg_unpredictable_data_pos);
                    block_pred_and_quant_regression_3d(z_data_pos, reg_params_pos, precision, recip_precision, capacity,
                                                       intv_radius,
                                                       size_x, size_y, size_z, size.dim0_offset, size.dim1_offset, type_pos,
                    ...
                    // Lorenzo
                    block_pred_and_quant_lorenzo_3d(mean_info, z_data_pos, pred_buffer, precision, recip_precision,
                                                    capacity_lorenzo, intv_radius,
...
template<typename T>
size_t
prediction_and_quantization_3d_with_border_prediction(const T *data, const DSize_3d &size, const meanInfo<T> &mean_info,
                                                      double precision,
                                                      int capacity, int intv_radius, unsigned char *indicator, int *type,
        ...
        for (size_t j = 0; j < size.num_y; j++) {
            ...
            for (size_t k = 0; k < size.num_z; k++) {
                ...
                if (*indicator_pos) {
                    ...
                    compress_regression_coefficient_3d(reg_precisions, reg_recip_precisions, reg_params_pos, reg_params_type_pos,
                                                       reg_unpredictable_data_pos);
                    block_pred_and_quant_regression_3d_with_buffer(z_data_pos, reg_params_pos, pred_buffer_pos, precision,
                    ...
                    // Lorenzo
                    block_pred_and_quant_lorenzo_3d(mean_info, z_data_pos, pred_buffer_pos, precision, recip_precision,
                                                    capacity_lorenzo, intv_radius,
                                                    size_x, size_y, size_z, buffer_dim0_offset, buffer_dim1_offset,
                                                    size.dim0_offset, size.dim1_offset, type_pos, unpredictable_data_pos);
                }
...
template<typename T>
size_t
prediction_and_quantization_3d_with_knl_optimization(const T *data, const DSize_3d &size, const meanInfo<T> &mean_info,
                                                     T precision,
                                                     int capacity, int intv_radius, unsigned char *indicator, int *type,
                                                     int *reg_params_type, float *&reg_unpredictable_data_pos,
        ...
        for (size_t j = 0; j < size.num_y; j++) {
            ...
            for (size_t k = 0; k < size.num_z; k++) {
                ...
                if (*indicator_pos) {
                    ...
                    compress_regression_coefficient_3d(reg_precisions, reg_recip_precisions, reg_params_pos, reg_params_type_pos,
                                                       reg_unpredictable_data_pos);
                    block_pred_and_quant_regression_3d_knl(z_data_pos, reg_params_pos, precision, recip_precision, capacity,
                                                           intv_radius,
                                                           size_x, size_y, size_z, size.dim0_offset, size.dim1_offset, type_pos,
                                                           unpred_count_buffer, unpred_data_buffer, offset);
                    reg_count++;
                    reg_params_pos += RegCoeffNum3d;
                    reg_params_type_pos += RegCoeffNum3d;
                } else {
                    // Lorenzo
                    block_pred_and_quant_lorenzo_3d_knl_2d_pred(mean_info, z_data_pos, pred_buffer, precision, recip_precision,
                                                                capacity_lorenzo, intv_radius,
                                                                size_x, size_y, size_z, buffer_dim0_offset, buffer_dim1_offset,
                                                                size.dim0_offset, size.dim1_offset, type_pos, unpred_count_buffer,
                                                                unpred_data_buffer, offset, 1);
                }
                indicator_pos++;
                z_data_pos += size_z;
            }
...
//                bool enable_regression = params.use_regression_linear && min_size >= 1;
                ...
                if (enable_regression) {
                    compute_regression_coeffcients_3d(cur_block_data_pos, size.block_size, size.block_size, size.block_size,
                                                      size.dim0_offset, size.dim1_offset,
                                                      reg_params_pos);
                }
                if (enable_poly) {
                    compute_regression_coeffcients_3d_poly(cur_block_data_pos, size.block_size, size.block_size,
                                                           size.block_size,
                                                           size.dim0_offset, size.dim1_offset,
                                                           reg_poly_params_pos, coef_aux_list);
                }
                int selection_result = sz_blockwise_selection_3d(cur_block_data_pos, mean_info, size.dim0_offset,
                                                                 size.dim1_offset,
                                                                 size.block_size, precision, reg_params_pos,
                                                                 reg_poly_params_pos,
                                                                 params.prediction_dim,
                                                                 params.use_lorenzo, params.use_lorenzo_2layer,
                                                                 enable_regression, enable_poly,
                                                                 params.poly_regression_noise);
                *indicator_pos = selection_result;
                if (selection_result == SELECTOR_REGRESSION_POLY) {
                    ...
                    block_pred_and_quant_regression_3d_with_buffer_knl(cur_block_data_pos, reg_poly_params_pos, pred_buffer,
                                                                       ...
                                                                       offset,
                                                                       params.lorenzo_padding_layer, true);
                    ...
                    block_pred_and_quant_regression_3d_with_buffer_knl(cur_block_data_pos, reg_params_pos, pred_buffer,
                                                                       ...
                                                                       type_pos, unpred_count_buffer, unpred_data_buffer,
                                                                       offset,
                                                                       params.lorenzo_padding_layer, false);
                    reg_count++;
                    reg_params_pos += RegCoeffNum3d;
                    reg_params_type_pos += RegCoeffNum3d;
                } else {
...
//    float *reg_poly_params = (float *) malloc(RegPolyCoeffNum3d * (size.num_blocks + 1) * sizeof(float));
    ...
    T *pred_buffer = (T *) malloc((size.block_size + params.lorenzo_padding_layer) * (size.d2 + params.lorenzo_padding_layer) *
    ...
    memset(pred_buffer, 0, (size.block_size + params.lorenzo_padding_layer) * (size.d2 + params.lorenzo_padding_layer) *
                           (size.d3 + params.lorenzo_padding_layer) * sizeof(T));
    int capacity_lorenzo = mean_info.use_mean ? capacity - 2 : capacity;
    auto *lorenzo_pred_and_quant = block_pred_and_quant_lorenzo_3d_knl_3d_pred_v2<T>;
    if (params.prediction_dim == 2) lorenzo_pred_and_quant = block_pred_and_quant_lorenzo_3d_knl_2d_pred_v2<T>;
    else if (params.prediction_dim == 1) lorenzo_pred_and_quant = block_pred_and_quant_lorenzo_3d_knl_1d_pred_v2<T>;
    T recip_precision = (T) 1.0 / precision;
    T *prediction_pos = prediction;
    ...
    for (size_t i = 0; i < size.num_x; i++) {
        ...
        for (size_t j = 0; j < size.num_y; j++) {
            const T *z_data_pos = y_data_pos;
            for (size_t k = 0; k < size.num_z; k++) {
                prediction_pos = prediction + (z_data_pos - data);
                ...
                *indicator_pos = selection_result;
                if (selection_result == SELECTOR_REGRESSION_POLY) {
                    ...
                    compress_regression_coefficient_3d_v2(RegPolyCoeffNum3d, reg_poly_precisions, reg_poly_recip_precisions,
                                                          reg_poly_params_pos, reg_poly_params_type_pos,
                                                          reg_poly_unpredictable_data_pos);
                    block_pred_and_quant_regression_3d_with_buffer_knl_v2(z_data_pos, reg_poly_params_pos, pred_buffer_pos,
                                                                       precision,
                                                                       recip_precision, capacity, intv_radius,
                                                                       size_x, size_y, size_z, buffer_dim0_offset,
                                                                       buffer_dim1_offset, size.dim0_offset, size.dim1_offset,
                                                                       type_pos, unpred_count_buffer, unpred_data_buffer, offset,
                                                                       params.lorenzo_padding_layer, true, prediction_pos);
                    ...
                    compress_regression_coefficient_3d_v2(RegCoeffNum3d, reg_precisions, reg_recip_precisions, reg_params_pos,
                                                          reg_params_type_pos,
                                                          reg_unpredictable_data_pos);
                    block_pred_and_quant_regression_3d_with_buffer_knl_v2(z_data_pos, reg_params_pos, pred_buffer_pos, precision,
                                                                       recip_precision, capacity, intv_radius,
                                                                       size_x, size_y, size_z, buffer_dim0_offset,
                                                                       buffer_dim1_offset, size.dim0_offset, size.dim1_offset,
                                                                       type_pos, unpred_count_buffer, unpred_data_buffer, offset,
                                                                       params.lorenzo_padding_layer, false, prediction_pos);
                    reg_count++;
                    reg_params_pos += RegCoeffNum3d;
                    reg_params_type_pos += RegCoeffNum3d;
                } else {
                    ...
                    lorenzo_pred_and_quant(mean_info, z_data_pos, pred_buffer_pos, precision, recip_precision, capacity_lorenzo,
                                           ...
                                           size.dim1_offset, type_pos, unpred_count_buffer, unpred_data_buffer, offset,
                                           params.lorenzo_padding_layer, (selection_result == SELECTOR_LORENZO_2LAYER),prediction_pos);
                    if (selection_result == SELECTOR_LORENZO_2LAYER) {
                        lorenzo_2layer_count++;
                    } else {
                        lorenzo_count++;
...
sz_compress_3d(const T *data, size_t r1, size_t r2, size_t r3, double precision, size_t &compressed_size,
    ...
    size_t reg_count = params.block_independant ? prediction_and_quantization_3d(data, size, mean_info, precision, capacity,
                                                                                 intv_radius, indicator, type, reg_params_type,
                                                                                 reg_unpredictable_data_pos,
                                                                                 unpredictable_data_pos)
                                                : prediction_and_quantization_3d_with_border_prediction(data, size, mean_info,
                                                                                                        precision, capacity,
                                                                                                        intv_radius, indicator,
                                                                                                        type, reg_params_type,
                                                                                                        reg_unpredictable_data_pos,
                                                                                                        unpredictable_data_pos);
...
template<typename T>
unsigned char *
sz_compress_3d_knl(const T *data, size_t r1, size_t r2, size_t r3, double precision, size_t &compressed_size,
                   const sz_params &params) {
    sz_compress_info compress_info;
...
template
unsigned char *
sz_compress_3d_knl<float>(const float *data, size_t r1, size_t r2, size_t r3, double precision, size_t &compressed_size,
                          const sz_params &params);


template<typename T>
unsigned char *
sz_compress_3d_knl_2(const T *data, size_t r1, size_t r2, size_t r3, double precision, size_t &compressed_size,
    ...
    float *reg_params = (float *) malloc(RegCoeffNum3d * (size.num_blocks + 1) * sizeof(float));
    float *reg_poly_params = (float *) malloc(RegPolyCoeffNum3d * (size.num_blocks + 1) * sizeof(float));
    T *prediction = (T *) malloc(size.num_elements * sizeof(T));
    ...
    prediction_and_quantization_3d_with_border_predicition_and_knl_optimization(data, size, mean_info,
                                                                                ...
                                                                                reg_poly_count, compress_info, reg_params, reg_poly_params, prediction);
    unsigned char *compressed = NULL;
    // TODO: change to a better estimated size
    size_t est_size = size.num_elements * sizeof(T) * 1.2;
    compressed = (unsigned char *) malloc(est_size);
...
//    verify((T*)data, prediction, size.num_elements);
#ifdef DUMP_PREDICTION_ERR_FOLDER
    char filename[300];
    string input_name(params.filename);
    auto input_names = split(input_name, '/');
    sprintf(filename, "%s/%s_%s_%.1e_l(%d)_l2(%d)_r(%d)_r2(%d).quan_bin.dat", DUMP_PREDICTION_ERR_FOLDER, input_names[input_names.size() - 2].data(),
    ...
    printf("write file %s\n", filename);
    writefile(filename, type, size.num_elements);

    sprintf(filename, "%s/%s_%s_%.1e_l(%d)_l2(%d)_r(%d)_r2(%d).pred.dat", DUMP_PREDICTION_ERR_FOLDER, input_names[input_names.size() - 2].data(),
            input_names[input_names.size() - 1].data(), params.eb, params.use_lorenzo,
            params.use_lorenzo_2layer, params.use_regression_linear, params.use_poly_regression);
    printf("write file %s\n", filename);
    writefile(filename, prediction, size.num_elements);
#endif
...
//        printf("num_elements = %ld, num_blocks = %ld\n", size.num_elements, size.num_blocks);
    ...
    T precision_t = (T) precision;
    size_t reg_count = 0, reg_poly_count = 0;
    prediction_3d_sampling(data, size, mean_info,
                           precision_t, capacity, intv_radius, indicator,
                           type, reg_params_type,
                           reg_unpredictable_data_pos,
                           reg_poly_params_type,
